"""Generate Deep Eval analysis reports under reports/.

Inputs (default fixed paths; may be overridden by CLI args):
  - deepeval/output/eval_req_vs_planning_v1.json
  - checklist/CHK-REQ-REVIEW-001.yaml
  - requirements/yaml/requirements_v1/*.yaml (loaded for indexing only)

Outputs:
  - reports/PRM-REQ-EVAL-002/deep_eval_summary.md
  - reports/PRM-REQ-EVAL-002/deep_eval_findings.csv
  - reports/PRM-REQ-EVAL-002/deep_eval_action_plan.md
  - reports/PRM-REQ-EVAL-002/deep_eval_patch_suggestions.md
  - reports/PRM-REQ-EVAL-002/deep_eval_runtime_issues.md

NOTE:
  - Default output dir is reports/PRM-REQ-EVAL-002.
  - You may override via --reports-dir.

Important:
  - This script does not modify any existing source requirement files.
  - All report assertions are grounded in Deep Eval "reason"/"error" text (no speculation).
"""
...
def build_requirement_index(req_dir: Path) -> Dict[str, Dict[str, str]]:
    ...
    Deep Eval JSON often includes source_file/doc_id. This is a fallback for missing fields.
    """
...
# Priority danger words (How-mixing) - we only *act* on them when present in reason/error.
DANGER_WORDS = [
    # DB / storage
...
def detect_issues(reason_or_error: str) -> List[DetectedIssue]:
    ...
    # Acceptance criteria missing / weak
    if (
    ...
    # Prompt structure / evaluation format missing
    if (
        "評価手順" in r
        or "スコアリング" in r
        or "0.0〜1.0" in r
    ...
    # Context completeness / unreachable references
    if (
        "到達不能" in r
    ...
    # Operational exceptions/logging
...
def choose_primary_item(issues: List[DetectedIssue], reason_or_error: str) -> str:
    """Mechanical mapping to checklist_item_id.

    Rules:
      1) If reason/error contains explicit CHK-XX-XX, use it.
      2) Else if issues detected, use the highest-weight issue mapping.
      3) Else unmapped => return "" (and report count in summary).
    """
...
def fix_hint_from(reason_or_error: str, issues: List[DetectedIssue]) -> str:
    ...
    if "how_mixing" in keys:
        ...
        if hits:
            hints.append(
                "How混入を除去: reason/errorで指摘された技術語（{}）を要件文から削除し、"
                "『外部から観測できる結果（表示/出力/通知/永続性）』に言い換える".format(", ".join(hits))
            )
        else:
...
def metric_base_shortfall(score: Any, threshold: Any, passed: bool) -> float:
    """base = max(0, threshold - score) for failed metrics; tolerant to missing values."""
    if passed:
        return 0.0
    # Spec base = max(0, threshold - score).
    # When score/threshold are missing but metric failed, use a small safe value
    # to avoid dropping items from priority ranking.
...
def metric_weight_from_issues(issues: List[DetectedIssue]) -> float:
    """Weight for a single metric row; grounded in detected issues from reason/error."""
    if not issues:
        return 1.0
    # Use max known weight among detected keys (stable & conservative)
    return max(ISSUE_WEIGHTS.get(i.key, 1.0) for i in issues)


def requirement_priority(failed_metrics: List[Dict[str, Any]]) -> float:
    """Aggregate priority from (base * weight) across failed metrics (already timeout-filtered)."""
...
def render_summary_md(
    ...
    # Top-5 lowest average metrics (numeric sort)
    ...
    if unmapped_checklist_item_count > 0:
        ...
        lines.append("")
        lines.append(
            f"- checklist_item_id を機械的に決定できない行が {unmapped_checklist_item_count} 件ありました。"
        )
        lines.append(
...
def render_action_plan_md(pattern_counts: List[Tuple[str, int]]) -> str:
    ...
    lines.append("")
    lines.append(f"- Top patterns: {top_patterns}" if top_patterns else "- Top patterns: (なし)")
    lines.append("")

    # Keep Day1〜Day7 structure; add Day0 as requested.
    lines.append("## Day0: runtime issue 切り分け（timeout/cancelled）")
    lines.append("- reports/PRM-REQ-EVAL-002/deep_eval_runtime_issues.md を確認")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate Deep Eval reports under reports/ (no source modifications).")
...
def main() -> int:
    ...
    # Count rows where checklist_item_id cannot be mechanically mapped
    unmapped_checklist_item_count = 0

    # pattern counts exclude timeout-related by design (to avoid skewing quality patterns)
    pattern_freq: Dict[str, int] = {}

    # per-requirement aggregation for Top10
    ...
    # Coverage evidence (grounded) from OVERALL metric reason/error
    ...
    for tr in test_results:
        ...
        for m in (tr.get("metrics") or []):
            ...
            # checklist_category normalization (purpose/scope/verifiability/ai_readability/traceability/schema/operational)
            checklist_category = ""
            if primary_item_id:
            ...
            # Update pattern frequency (exclude runtime issues)
            ...
            # Ensure either reason or error is populated (requirement)
            if (not row.get("reason")) and (not row.get("error")):
            ...
            # Requirement-level aggregation (exclude timeout-related failures)
            if requirement_id and (not passed) and (not timeout_related):
                ...
                per_req_patterns.setdefault(requirement_id, []).extend([i.key for i in issues])
                if requirement_id not in per_req_evidence and reason_or_error:
                    per_req_evidence[requirement_id] = short_text(reason_or_error, 180)

    # Metric averages
    summary = eval_data.get("summary", {}) or {}
    metric_avgs = (summary.get("metric_averages", {}) or {})

    # Coverage notes if avg is low/0.0
    coverage_notes: List[str] = []
    for k, v in metric_avgs.items():
        name = str(k)
        try:
    ...
    # Top10 requirement ranking (timeout由来除外済みの失敗のみで算出)
    ...
    for req_id, rows in per_req_failed_metric_rows.items():
        ...
        patt = per_req_patterns.get(req_id, []) or []
        patterns_hr: List[str] = []
        # show most important patterns first
        for kk in sorted(set(patt), key=lambda kkk: (-ISSUE_WEIGHTS.get(kkk, 0.0), kkk)):
            patterns_hr.append(issue_label(kk))
    ...
    # Write outputs
    write_findings_csv(findings_rows, args.reports_dir / "deep_eval_findings.csv")
    ...
    (args.reports_dir / "deep_eval_action_plan.md").write_text(
        render_action_plan_md(pattern_counts_sorted),
        encoding="utf-8",
    )
    ...
    checks = {
        "E_runtime_issues_count": len(runtime_issues),
        "A_top10_excludes_timeout": (not top10_has_timeout),
        "B_csv_has_error_column": ("error" in CSV_FIELDS),
        "B_csv_has_is_timeout_related_column": ("is_timeout_related" in CSV_FIELDS),
    }
    print("[SELF-CHECK]")
