...
AI駆動開発の導入により、コード生成・設計支援が急速に進展している。
しかし、現場では深刻な品質課題が顕在化している。
...
| 仕様と実装の乖離 | 要件定義・設計書・実UIが一致しない   | テスト工数の爆発的増加、手戻り多発   |
...
NotebookLMのマインドマップUIを参考に、直感的な操作で品質保証活動を実施できる。
...
すべての成果物に一意のIDを付与し、紐付けを記録する。

| 成果物種別     | IDプレフィックス | 例                                  |
| -------------- | ---------------- | ----------------------------------- |
| 要件           | REQ-             | REQ-F001（機能要件1）               |
| 基本設計       | BAS-             | BAS-D001（設計項目1）               |
| 詳細設計       | DET-             | DET-C001（コンポーネント1）         |
| テストケース   | TST-             | TST-R001（要件F001のテスト）        |
| プロンプト     | PRM-             | PRM-REQ-001（要件定義用プロンプト） |
| チェックリスト | CHK-             | CHK-REQ-AMB（曖昧語チェック）       |
| テスト結果     | RES-             | RES-TST-R001-001（テスト実行結果）  |
...
traceability:
  REQ-F001:
    title: "ユーザーログイン機能"
    tracesto:
      - BAS-D001  # 基本設計：認証フロー設計
      - DET-C001  # 詳細設計：LoginComponentの実装仕様
      - TST-R001  # テストケース：ログイン正常系
      - TST-R002  # テストケース：ログイン異常系
    promptsused:
      - PRM-REQ-001  # 使用したプロンプト
    evaluation:
...
checkpoints: - id: "CHK-REQ-AMB-001"
...
| 判断の重要度 | 記述粒度                   | 例                                   |
| ------------ | -------------------------- | ------------------------------------ |
| High         | 詳細な根拠と代替案検討記録 | アーキテクチャ選定、データモデル設計 |
...
acceptancecriteria: - "生成されたワークフローがスキーマ検証を100%パス" - "手動作成と自動生成で同じUIレンダリング" - "人間が読んで違和感がない"
`
...
functional: - "すべての画面が設計通りに動作" - "データの保存・読込が正常" - "エクスポート機能が動作"
